...
function normalizeMonthly(monthly = []) {
  ...
  monthly.forEach((row) => {
    map.set(row.key || row.label, row);
  });

  return defaultMonths.map((month) => {
    const row = map.get(month.key) || map.get(month.label) || {};
...
export default function MasterDashboard() {
  ...
  const monthlyRows = useMemo(
    () => normalizeMonthly(data.monthly),
    [data.monthly]
  );

  const monthlyTotalValues = useMemo(() => {
  ...
  const categoryTotalValues = useMemo(() => {
    const totalInwardValue = (data.categorySummary || []).reduce(
      (sum, row) => sum + num(row.inwardValue),
      0
    );

    const totalIssuanceValue = (data.categorySummary || []).reduce(
      (sum, row) => sum + num(row.issuanceValue),
      0
    );
  ...
  const totals = useMemo(() => {
    const totalInwardValue =
      num(data.totals?.totalInwardValue) ||
      categoryTotalValues.totalInwardValue ||
      monthlyTotalValues.totalInwardValue;

    const totalIssuanceValue =
      num(data.totals?.totalIssuanceValue) ||
      categoryTotalValues.totalIssuanceValue ||
      monthlyTotalValues.totalIssuanceValue;
    ...
    return {
      ...
      totalIssuanceValue,
      issuanceGap:
        num(data.totals?.issuanceGap) ||
        totalIssuanceValue - totalInwardValue,
    };
  }, [data.totals, categoryTotalValues, monthlyTotalValues]);
  ...
  const maxMonth = useMemo(() => {
    return Math.max(
      1,
      ...monthlyRows.map((row) =>
        Math.max(num(row.inwardTotal), num(row.issuanceTotal))
      )
    );
  }, [monthlyRows]);
